...
def show_result():
    for p_name in player_info:
        best_players.append(
            {"name": p_name, "total_score": sum(player_info[p_name].values())}
        )
    for show in sorted(
        best_players, key=lambda item: (-item["total_score"], item["name"])
    ):
        print(f"{show['name']}: {show['total_score']} skill")
        for pos, skill in sorted(
            player_info[show["name"]].items(), key=lambda item: (-item[1], item[0])
        ):
            print(f"- {pos} <::> {skill}")
