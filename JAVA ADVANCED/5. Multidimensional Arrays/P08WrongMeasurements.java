...
public class P08WrongMeasurements {
        ...
        for (int r = 0; r < matrix.length; r++) {
            for (int c = 0; c < matrix[r].length; c++) {
                if (matrix[r][c] == wrongValue) {
                    int sum = 0;
                    // Top
                    if (isBounds(matrix, r - 1, c) && matrix[r - 1][c] != wrongValue) {
                        sum += matrix[r - 1][c];
                    }
                    // Left
                    if (isBounds(matrix, r, c - 1) && matrix[r][c - 1] != wrongValue) {
                        sum += matrix[r][c - 1];
                    }
                    // Down
                    if (isBounds(matrix, r + 1, c) && matrix[r + 1][c] != wrongValue) {
    ...
    private static boolean isBounds(int[][] matrix, int r, int c) {
