# Reshenie ot chatgpt s Dictionary

# Създаване на речник с цените за продуктите в различните градове
#prices = {
#    "Sofia": {"coffee": 0.50, "water": 0.80, "beer": 1.20, "sweets": 1.45, "peanuts": 1.60},
#    "Plovdiv": {"coffee": 0.40, "water": 0.70, "beer": 1.15, "sweets": 1.30, "peanuts": 1.50},
#    "Varna": {"coffee": 0.45, "water": 0.70, "beer": 1.10, "sweets": 1.35, "peanuts": 1.55}
#}

# Въвеждане на продукт, град и количество от потребителя
#product = input()
#city = input()
#quantity = float(input())

# Проверка дали градът и продуктът са в речника с цени
#if city in prices and product in prices[city]:
#    price_per_unit = prices[city][product]
#    total_price = price_per_unit * quantity
#    print(f"{total_price:.2f}")
#else:
#    print("Invalid input")












